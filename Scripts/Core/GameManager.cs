    ...
    public class GameManager : MonoBehaviour
    {
        ...
        private IEnumerator ExecuteMove(Move move)
        {
            ...
            if (result.IsCapture)
            {
                ...
                {
                    // Cinematic capture: move attacker to target, play battle, then remove defender
                    // Remove defender from PieceController dict first so attacker can take its position
                    pieceController.RemovePiece(move.To);

                    yield return StartCoroutine(pieceController.MovePieceTo(move.To));
                    pieceController.DeselectPiece();

                    // Play cinematic battle animation
